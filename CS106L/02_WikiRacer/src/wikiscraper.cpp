...
unordered_set<string> findWikiLinks(const string& page_html) {
    std::string link = "<a href=\"/wiki/";
    unordered_set<string> wiki_links;
    auto it = page_html.begin();
    while(true) {
        auto start_it = std::search(it, page_html.end(), link.begin(), link.end());
        if (start_it == page_html.end()) break;
        else {
            size_t start = start_it + link.size() - page_html.begin();
            size_t end = page_html.find('"', start);
            string target = page_html.substr(start, end - start);
            bool valid = std::all_of(target.begin(), target.end(), isValid);
            if (valid) {
                wiki_links.insert(target);
            }
            it = page_html.begin() + end;
        }
    }
    return wiki_links;
